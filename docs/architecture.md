...
![SSE 멀티디바이스 동기화](images/sse-sync.png)
...
- dev는 `main` push로 자동 배포하되, 워크플로우의 `paths` 필터로 실제 바뀐 영역만 돈다 — `backend/**`·`infra/dev/**` 변경이면 BE만, `frontend/**` 변경이면 FE만 배포된다 (둘 다 바뀌면 동시에). `docs/`만 수정한 push는 어느 쪽도 트리거하지 않는다.
