...
- **현재 범위**: **일간/주간/월간 회고 활성화**
- **구성**:
  - 탭: 일일/주간/월간
  - 기간 네비게이터: 좌/우 이동
  - 요약 카드: 집중 시간, Flow 세션, 완료 태스크
  - 일일: 완료 → 미완료 순서 리스트 (작업 페이지 순서 기준, 체크 유무로 구분)
  - 주간: 날짜별 세로 타임라인(접기/펴기, 기본 전체 펼침)
  - 월간: 주차별 세로 타임라인(접기/펴기, 기본 전체 펼침)
    - 주차 내부에서 날짜 구분선(예: `2/5(목)`) 아래에 완료/미완료를 함께 리스트업
  - 회고 입력: 인라인 읽기/편집 모드
    - 읽기 모드: 텍스트 그대로 표시
    - 편집 모드: `삭제/완료` 버튼
    - 외부 클릭 시 편집 취소
