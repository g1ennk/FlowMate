...
| 방식                | 장점                                     | 단점                                            | 판단     |
|-------------------|----------------------------------------|-----------------------------------------------|--------|
| Sticky Session    | 별도 메시지 브로커 없이 특정 클라이언트를 같은 인스턴스로 고정 가능 | 멀티디바이스, 장애, 재배포 상황에서 이벤트 전파 문제를 근본적으로 해결하지 못함 | 임시 완화  |
| Kafka             | 메시지 보장, 파티션 내 순서 보장, replay 지원         | 현재 요구사항 대비 운영과 구성 복잡도가 높음                     | 과잉     |
...
로컬 2-JVM 환경에 부하를 걸어 도달률, 유실, 지연을 정량으로 측정했다. SSE 클라이언트는 인스턴스 B(`:8081`)에만 연결하고 PUT은 인스턴스 A(`:8080`)에만 보내 순수 인스턴스 간
경로만 측정하도록 고정했다.
...
| 결정                       | 얻은 것                                             | 감수한 비용                            | 판단                                  |
|--------------------------|--------------------------------------------------|-----------------------------------|-------------------------------------|
| **Redis Pub/Sub 채택**     | 단순한 구조로 인스턴스 간 이벤트 전파 가능                         | at-most-once 특성상 메시지 유실 가능        | 최종 상태는 MySQL에 있으므로 유실이 곧 데이터 손실은 아님 |
...
로컬 2-JVM 환경에서 인스턴스 간 도달률을 0%에서 100%로 개선할 수 있었다.
