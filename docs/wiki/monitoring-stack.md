...
Grafana Cloud를 쓰면 역할이 분리된다. EC2는 Alloy가 데이터를 수집하고, Grafana Cloud가 저장과 시각화를 담당한다. Nginx에서 Grafana UI를 직접 노출하지 않는다.
...
| 지표 확인   | JVM heap과 GC 여유, slow query 0건, host CPU 100% |
| 검증 (v4) | pool 10->20으로 변경 후 재측정 -> throughput 25%↓     |
| 원인      | pool이 아니라 host 스펙 자체가 한계                      |

pool과 thread 튜닝은 제외하고, 인스턴스 스펙업이나 DB hit 감소(Redis, MySQL 분리) 쪽으로 방향을 잡았다.

## 7. 회고

### 복잡도는 더 올라가기 전에 줄일 수 있을 때 줄이는 편이 좋다

Self-hosted Prometheus + Grafana는 잘 동작했다. 문제는 그 다음이었다.
...
결국 Observability를 제대로 활용하려면 도구보다 먼저 지표에 대한 이해가 필요하다는 것을 느꼈다. 각 지표가 무엇을 의미하는지, 정상 범위와 이상 신호는 어떻게 구분하는지, 하나의 지표 변화가 다른
지표들과 어떤 연관관계를 가지는지 알아야 문제를 더 정확히 좁히며 파악할 수 있을 것이다.
